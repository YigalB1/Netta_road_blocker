...
//rotate([0,0,90]) translate([0,-10,-box_h/2+3]) arduino_shield_base(usb_dummy="false");
...
module box_to_print(inner="false",ext_pins="false") {
    ...
    difference() {        
        union() {
            box(ext_pins);
            rotate([0,0,90]) translate([0,-10,-box_h/2+3])
                arduino_shield_base(usb_dummy="false");
        } // of union
        
        translate(servo_vec) servo();
        //translate(us_sensor_vec) US_sensor(); // module was not accurate
        translate(us_sensor_vec) ultra_sonic();
        rotate([0,0,90]) translate([0,-10,-box_h/2+3+9.3]) arduino_shield_base(usb_dummy="true"); // just the USB connector for the hole in the box
